...
export function mayStartViewTransition(param, extensions = {}, scope = document) {
    ...
    function batch() {
        ...
        currentViewTransition.ready.then(() => updated.forEach((update) => update.readyResolve()), () => updated.forEach((update) => update.readyReject()));
        currentViewTransition.finished
            .then(() => updated.forEach((update) => update.finishResolve()), () => updated.forEach((update) => update.finishReject()))
            .finally(() => {
            updated.length = 0;
